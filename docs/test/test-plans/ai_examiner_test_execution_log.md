...
- **Kết quả:** Mã lỗi trả về là gì? `[404 Not Found]` -> `[PASS]`
...
- **Kết quả:** Database có tạo 2 turn không? `[Không. Nhưng cả 2 request đều bị văng mã lỗi 409 Conflict và không có lượt nào được tạo ra trong DB. Lẽ ra phải có 1 request thành công 200 OK và request kia bị chặn/trả về cache. Lỗi Logic API.]` -> `[FAIL]`
...
| **TC-010** | Bảo mật (Phân quyền truy cập API)         | Trả về `404 Not Found` đúng thiết kế.                                       |   `PASS`   |          `[N/A]`          |
| **TC-011** | Idempotency (Gửi đúp POST /answers)       | Cả 2 request đều bị lỗi `409 Conflict`. Không tạo lượt trong DB.            |   `FAIL`   |     `[Sẽ tạo Issue]`      |
