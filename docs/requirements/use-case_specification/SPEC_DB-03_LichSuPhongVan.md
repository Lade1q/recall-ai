...
7. **Bản ghi hỏi-đáp và cách tính điểm:** Với mỗi khái niệm, hệ thống hiển thị được từng lượt (`interview_turns`): câu hỏi, câu trả lời của Sinh viên, điểm (`score`), phân loại (`verdict`) và nhận xét (`feedback`) của từng lượt, cùng phép tính trung bình có trọng số ra `mastery_score` cuối cùng của khái niệm. Trọng số mặc định là `[0.2, 0.3, 0.5]` cho ba lượt vì lượt sau sâu hơn lượt trước (UC-Overview §5.4).
