...
- **Hàng đợi rỗng cho plan mới (chưa từng có phiên Interview nào):** trả `items` là gợi ý trực
  ...
  `reason: "spaced_repetition"`, `reasonText: "Khái niệm chưa được kiểm tra"` và `id: null` (vì
  chưa có row `ReviewQueueItem` thật — không thể `PATCH` các item này).

- **Đã ôn hết / plan hết item pending (HTTP 200 OK, không phải lỗi):**

  ```json
  { "success": true, "data": { "items": [], "message": "Bạn đã hoàn thành kế hoạch hôm nay 🎉" } }
  ```

- **Plan chưa ở trạng thái `active` (HTTP 200 OK, không phải lỗi):**

  ```json
  { "success": true, "data": { "items": [], "message": "Kế hoạch chưa ở trạng thái hoạt động." } }
  ```
...
- **Response thành công (HTTP 200 OK):** shape giống hệt mục 1 (`{ items, message }`), nhưng
  `items` gộp từ nhiều plan.
...
- **User chưa có plan nào đang `active` (HTTP 200 OK, không phải lỗi):**
  ...
  {
    "success": true,
    "data": { "items": [], "message": "Bạn chưa có kế hoạch ôn tập nào đang hoạt động." }
  }
  ```
