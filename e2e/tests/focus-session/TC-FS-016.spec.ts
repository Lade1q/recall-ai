...
test.describe('TC-FS-016: Khôi phục phiên bị gián đoạn sau khi đóng tab', () => {
  test('a) Ghi nhận snapshot hoàn tất đúng record rồi xóa dữ liệu khôi phục', async ({
    ...
    try {
      ...
      await expect(
        recoveryPage.getByText(`Đã ghi nhận ${Math.floor(focusedSeconds / 60)} phút tập trung.`, {
          exact: true,
        })
      ).toBeVisible();
  ...
  test('b) Bỏ qua không hoàn tất record, xóa snapshot và không hỏi lại', async ({
    ...
    try {
      ...
      // 3. Không gửi PATCH, không tạo completed; record gốc vẫn là audit running chờ lazy-reap.
      expect(patchRequests).toHaveLength(0);
      expect(
        await prisma.focusSession.findUniqueOrThrow({
          where: { id: sessionId },
          select: { status: true, endedAt: true, durationMinutes: true },
        })
      ).toEqual({ status: 'running', endedAt: null, durationMinutes: 0 });
      expect(
        await recoveryPage.evaluate((key) => localStorage.getItem(key), snapshotKey)
      ).toBeNull();
